...
search.addEventListener("input", async (event) => {
  ...
  //   const urlPokemon = `https://pokeapi.co/api/v2/pokemon/${pokemonValue}`;
  //   const urlPokemonResponse = await fetch(urlPokemon);
  //   const urlPokemonObj = await urlPokemonResponse.json();
    
  //   console.log(response);

  //   let pokeObjMap = pokeObjNoLimit.results.map((item) => {
  //     let pokeNome = item.name;

  //     if (pokeNome.startsWith(pokemonValue)) {
  //       console.log("Match:", pokeNome);
  ...
  //       optionsImg.setAttribute("src", `${urlPokemonObj.sprites.front_default}`);
